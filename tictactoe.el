...
(defvar *tictactoe-current-player* nil
  "The character indicating the current player.")
...
(defun tictactoe-game-has-been-won ()
  "Returns t if the game has been won, and nil otherwise."
...
(defun tictactoe-row-win ()
  (let ((has-won nil))
    (dotimes (current-row *tictactoe-size*)
      (when (tictactoe-all-same-player (tictactoe-get-square current-row 0)
                                       (tictactoe-get-square current-row 1)
                                       (tictactoe-get-square current-row 2))
        (setq has-won t)))
    has-won))

(defun tictactoe-column-win ()
  (let ((has-won nil))
    (dotimes (current-column *tictactoe-size*)
      (when (tictactoe-all-same-player (tictactoe-get-square 0 current-column)
                                       (tictactoe-get-square 1 current-column)
                                       (tictactoe-get-square 2 current-column))
        (setq has-won t)))
    has-won))
