...
#+begin_src bash
  ...
  ./configure --with-cairo --with-native-compilation  #--with-xwidgets
...
#+begin_src emacs-lisp
  ...
  (key-chord-define-global "ç "  'whitespace-cleanup) ; 'my-whitespace-cleanup adds empty line at end.
...
In case we want to open an MR we can now do so with the interactive `(magit-open-mr)`:
#+begin_src emacs-lisp
(load "magit-open-mr")
#+end_src
