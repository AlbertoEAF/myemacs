...
** Setup ~myemacs-dir~ variable
#+begin_src emacs-lisp
  ;; Crucial variable to search local modules:
  (if (boundp 'myemacs-dir)
      (unless (string-suffix-p "/" myemacs-dir)
        (setq myemacs-dir (concat myemacs-dir "/")))
...
#+begin_src emacs-lisp
  (use-package pdf-tools
    :ensure t
    :if (equal system-type 'gnu/linux) ;; disable on Windows for now until fix is available.
    :config
    (pdf-tools-install)
